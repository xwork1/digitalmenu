			...
			btnLogin.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					 if (fld_user.getText().length() == 0 || fld_pass.getText().length() == 0) {
		                    ...
		                    ConnectDB conn = factory.CreateConnection();
		                    Command command = factory.CreateCommand();
		                    try {
		                        Connection con = (Connection) conn.Connect();
		                        Statement st = con.createStatement();
		                        String query = "Select * from yetkili";
		                        ResultSet rs = command.Read(st, query);
		                        while (rs.next()) {
		                            if (fld_user.getText().equals(rs.getString("username")) && fld_pass.getText().equals(rs.getString("password"))) {
		                                ...
		                                admin.setLog(rs.getString("log"));
		                                admin.setName(rs.getString("name"));
		                                System.out.println(admin.getLog()+": "+ admin.getName());
		                                dispose();
