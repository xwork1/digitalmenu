...
import Main.Menu;
import Odemeyontemi.KrediPayGUI;
...
public class SepetGUI extends JFrame {
	...
	public void guncelle() throws SQLException {
		...
		while(rs3.next()){
			...
			yemekData[1] = rs3.getString("sepet_adi");
			yemekData[2] = rs3.getString("sepet_fiyati");
    ...
    try {
    		...
    		while(rs.next()){
    			...
    			yemekData[1] = rs.getString("sepet_adi");
    			yemekData[2] = rs.getInt("sepet_fiyati")+" TL";
    			yemekModel.addRow(yemekData);
    		}
    		table_sepet.setModel(yemekModel);
		...
		btnDelButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
		        ...
		        try {
		        	...
		        	ResultSet rs = sorgu.executeQuery("SELECT SUM(sepet_fiyati) AS fiyat FROM sepet");
		            while(rs.next()) {
		    			String yemekfiyati = rs.getInt("fiyat")+"TL";
		    			editorPane.setText(yemekfiyati);
		    		}
		        }catch(Exception e) {
    ...
    btnSatinal.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
            new KrediPayGUI().setVisible(true);
			/*Connection con2 = conn.Connect();
	        try {	        
	        	Statement sorgu = con2.createStatement();
