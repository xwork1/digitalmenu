...
public class LoginGUI extends JFrame {
	...
	private MariaDBConnection conn = new  MariaDBConnection();
	...
	private Object[] kampanyaList = {"ID","Yemek Adý","Fiyati"};
	
public void guncelle1() throws SQLException {
		
		Connection con44 = conn.Connect();
		Statement st33 = con44.createStatement();
        ResultSet rs33 = st33.executeQuery("SELECT * FROM kampanyalar order by id DESC LIMIT 1");
        
		while(rs33.next()){
			kampanyaList[0] = rs33.getInt("id");
			kampanyaList[1] = rs33.getString("kampanya_adi");
			kampanyaList[2] = rs33.getString("kampanya_fiyati");
			kampanyaModel.addRow(kampanyaList);
		}
    ...
    try {
        	...
        	kampanyaModel.setColumnIdentifiers(kampanyaList);
        	kampanyaList = new Object[3];
    		while(rs.next()){
    			kampanyaList[0] = rs.getInt("id");
    			kampanyaList[1] = rs.getString("kampanya_adi");
    			kampanyaList[2] = rs.getString("kampanya_fiyati")+" TL";
    			kampanyaModel.addRow(kampanyaList);
    		}
    		kampanya_table.setModel(kampanyaModel);
    ...
    public void actionPerformed(ActionEvent arg0) {
    MariaDBConnection db2 = new MariaDBConnection();
    Connection c5 = db2.Connect();
    try {
